    ...
    <section className="scroll-mt-24" id="about">
      ...
      <div className="grid items-center gap-16 lg:grid-cols-[minmax(320px,566px)_minmax(0,631px)] xl:gap-[100px]">
        ...
        <div className="grid justify-items-start gap-7">
          ...
          <a className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-brand to-[#e46400] px-10 py-3 font-bold text-white transition hover:-translate-y-0.5" href={contact.cv} download>
            <Download size={18} />
            Download CV
          </a>
        </div>
      ...
      <div className="mt-24 grid grid-cols-2 gap-8 md:grid-cols-3 xl:grid-cols-6">
        {skills.map((skill) => (
          <div className="grid justify-items-center gap-3 text-center" key={skill.name}>
            <div className="grid h-[130px] w-[130px] place-items-center rounded-full border-2 border-brand bg-white/[0.04] shadow-[inset_0_0_0_10px_rgba(253,111,0,0.08)]">

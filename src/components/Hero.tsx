    ...
    <section className="grid scroll-mt-24 items-center gap-16 lg:min-h-[781px] lg:grid-cols-[minmax(0,534px)_minmax(420px,1fr)]" id="home">
      <div className="grid justify-items-center gap-6 text-center motion-safe:animate-fade-up lg:justify-items-start lg:text-left">
        ...
        <div className="flex w-full flex-col items-center gap-4 sm:w-auto sm:flex-row">
          <a className="inline-flex min-h-12 w-full items-center justify-center rounded-lg bg-gradient-to-r from-brand to-[#e46400] px-10 py-3 font-bold text-white shadow-lg shadow-brand/20 transition duration-300 hover:-translate-y-1 hover:shadow-brand/35 sm:w-auto" href={`mailto:${contact.email}`}>
            Hire Me
          </a>
          <a className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-lg border-2 border-muted px-10 py-3 font-bold text-muted transition duration-300 hover:-translate-y-1 hover:border-brand hover:text-brand sm:w-auto" href={contact.cv} download>
            <Download size={18} />
            Download CV
          </a>
        </div>

        <div className="mt-9 flex w-full flex-col gap-0 rounded-lg bg-white/[0.04] p-6 shadow-2xl shadow-black/10 transition duration-300 hover:bg-white/[0.06] sm:w-fit sm:flex-row sm:gap-8">
          {stats.map((stat) => (
